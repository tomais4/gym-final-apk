...
# --- DADOS PADRÃO (HARDGAINER) ---
TREINOS_PADRAO = {
    "A: EMPURRAR (Peito/Ombro/Tri)": [
        {"nome": "Supino Inclinado c/ Halteres", "series": [{"tipo": "TOP", "min": "6", "max": "9"}, {"tipo": "BACK-OFF", "min": "6", "max": "9"}]},
...
class BancoDeDados:
    ...
    def carregar_dados(self):
        if not self.page.client_storage.contains_key("gym_data"):
    ...
    def obter_ultimo(self, nome_ex, idx):
        for s in reversed(self.dados["historico_log"]):
            for e in s["exercicios"]:
                if e["nome"] == nome_ex:
                    try: return e["series_realizadas"][idx]
                    except: pass
        return None
...
def main(page: ft.Page):
        ...
        page.add(
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("FDC MALUCO", color=COR_TEXTO_DIM, weight="bold", size=14),
                    ...
                    ft.Text("V19 Mobile Stable", color="#333", size=10)
    ...
    def selecao():
        ...
        page.add(
            ft.Column([
                ...
                ft.Divider(color=COR_SECUNDARIA),
                ft.Container(content=col_treinos, expand=True, padding=bottom=20),
    ...
    def executar(nome):
        ...
        def finalizar(e):
            ...
            db.salvar_dados()
            page.show_snack_bar(ft.SnackBar(ft.Text("Treino Finalizado! FDC MALUCO!"), bgcolor="green"))
            mudar_tela("menu")

        page.add(
            ft.Column([
                ft.Container(content=ft.Text(f"TREINANDO: {nome.split(':')[0]}", color=COR_PRIMARIA, weight="bold", size=18), padding=ft.padding.only(top=10, bottom=10), alignment=ft.alignment.center),
                lista_exercicios,
                ft.Container(content=ft.ElevatedButton("FINALIZAR TREINO", bgcolor=COR_PRIMARIA, color="white", width=300, height=50, on_click=finalizar), padding=20, alignment=ft.alignment.center)
            ], expand=True)
        )
